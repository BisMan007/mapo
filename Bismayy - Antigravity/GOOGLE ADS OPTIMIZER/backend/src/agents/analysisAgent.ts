...
export class AnalysisAgent {
  ...
  static async syncAllReports(): Promise<{ apiOperations: number; timestamp: string }> {
    ...
    // 1. Fetch search keyword report
    const keywords = await searchGoogleAds(`
      SELECT 
    ...
    // 2. Fetch search term report
    const searchTerms = await searchGoogleAds(`
      SELECT 
        campaign.name, 
        ad_group.name, 
    ...
    // 3. Fetch location report
    const locations = await searchGoogleAds(`
      SELECT 
        campaign.name, 
        metrics.impressions, 
        metrics.clicks, 
    ...
    // 4. Fetch device report
    const devices = await searchGoogleAds(`
      SELECT 
        campaign.name, 
        metrics.impressions, 
        metrics.clicks, 
    ...
    // 5a. Fetch age range demographics
    const ageDemographics = await searchGoogleAds(`
      SELECT 
        campaign.name, 
        metrics.impressions, 
        metrics.clicks, 
    ...
    // 5b. Fetch gender demographics
    const genderDemographics = await searchGoogleAds(`
      SELECT 
        campaign.name, 
        metrics.impressions, 
        metrics.clicks, 
    ...
    // 6. Fetch landing page report
    const landingPages = await searchGoogleAds(`
      SELECT 
        campaign.name, 
        metrics.impressions, 
        metrics.clicks, 
    ...
    // 7. Expanded landing page report
    const expandedLPs = await searchGoogleAds(`
      SELECT 
    ...
    // 8. Ad performance report
    const ads = await searchGoogleAds(`
      SELECT 
        ad_group_ad.ad.id,
        ad_group_ad.status,
        campaign.name,
    ...
    // 9. Auction Insights (Bypassed live query since Auction Insights is whitelisted-only via API)
    await db.exec('DELETE FROM r_auction_insights');
    
    // Fallback Mock data for Auction Insights to keep Dashboard and Competitor Benchmarks functional
    const mockAuctions = [
    ...
    for (const au of mockAuctions) {
      ...
      // Populate competitive domain index
      await db.run(
    ...
    // 10. Hour of Day
    const hours = await searchGoogleAds(`
      SELECT 
        campaign.name,
        metrics.impressions,
        metrics.clicks,
    ...
    // 11. Day of Week
    const days = await searchGoogleAds(`
      SELECT 
        campaign.name,
        metrics.impressions,
        metrics.clicks,
    ...
    // 12. PMax Placements
    const placements = await searchGoogleAds(`
      SELECT 
    ...
    // 13. Audiences Report
    const audiences = await searchGoogleAds(`
      SELECT 
        campaign.name,
        metrics.impressions,
        metrics.clicks,
    ...
    // 15. Assets/Creatives Report
    const assets = await searchGoogleAds(`
      SELECT 
