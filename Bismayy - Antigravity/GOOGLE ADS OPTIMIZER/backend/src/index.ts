...
// DIAGNOSTIC API
// ==========================================
app.get('/api/diag/test-queries', async (req, res) => {
  const queries = [
    {
      name: "keywords",
      query: `
  ...
  try {
    ...
    for (const q of queries) {
      try {
        const data = await searchGoogleAds(q.query);
        results.push({ name: q.name, success: true, count: data.length });
      } catch (err: any) {
        results.push({
          name: q.name,
          success: false,
